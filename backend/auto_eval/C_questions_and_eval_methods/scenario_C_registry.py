...
"""Scenario C registry and criterion catalog prototype."""
...
QUESTION_FILE = Path(__file__).with_name("C_scenario_questions.json")
CRITERIA_PER_TURN = 3
...
def build_default_criteria() -> list[CriterionSpec]:
    from scenario_C_llm_judges import choose_prompt_ref
    ...
    for session_index, session, turn in iter_turns():
        ...
        base = "C-S{:03d}-T{:02d}".format(session_index, turn["turn_id"])
        criteria.append(
            CriterionSpec(
                ...
                title="content-rule",
                implementation_entrypoint="scenario_C_content_checks.evaluate_content_rule",
        ...
        criteria.append(
            CriterionSpec(
                ...
                implementation_entrypoint="scenario_C_runbook.evaluate_trace_and_artifact",
        ...
        criteria.append(
            CriterionSpec(
                ...
                title="llm-judge",
                implementation_entrypoint="scenario_C_llm_judges.build_llm_prompt",
    ...
    if len(criteria) != expected_count:
        raise AssertionError(
            f"Scenario C should expose {expected_count} criteria, got {len(criteria)}"
        )
    missing = validate_file_coverage()
    if missing:
        raise AssertionError(f"Scenario C is missing declared uploads for: {missing}")
    return criteria
